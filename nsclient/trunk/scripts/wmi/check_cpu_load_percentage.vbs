...
'-------------------------------------------------------------------------------------------------
	Function f_Help()
		...
		Dim str
  		str="Check CPU Load Percentage. If your Local Machine has the same Administrator account and password as the Remote Machine then you don't have to use the two last parameters."&vbCrlF&vbCrlF
  		str=str&"cscript check_cpu_load_percentage.vbs -h hostname -inst DeviceID [-w warning_level -c critical_level] [-t thresoldpairs] [-user username -pass password [-domain domain]]"&vbCrlF
  		str=str&vbCrlF
  		str=str&"-h [--help]                 Help."&vbCrlF
  		str=str&"-h hostname                 Host name."&vbCrlF  
  		str=str&"-inst processors            DeviceID Instance."&vbCrlF  
  		...
  		str=str&"-t thresold pairs           Threshold pairs for each processor. Note: each thresold pair separated by a ':', warning value and critical separated by ',' character"&vbCrlF
  		...
  		str=str&vbCrlF
  		str=str&"Example: cscript check_cpu_load_percentage.vbs -h Ser1 -inst ""*""  -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
  		str=str&"Example: cscript check_cpu_load_percentage.vbs -h Ser1 -inst ""CPU0, _Total""  -w 30 -c 70 [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
  		str=str&"Example: cscript check_cpu_load_percentage.vbs -h Ser1 -inst ""CPU0, _Total""  -t ""70,90:60,80"" [-user Ser1\Administrator -pass password -domain ITSP] " &vbCrlF
  		wscript.echo str
	...
	Function f_GetInstance(instance)
		...
		if(instance = "*") then 
			For Each objWMI in colWMI
				...
				strPropName = "Load Percentage"
				intValue = objWMI.Properties_("LoadPercentage")
				f_Display()
			...
			strName = "_Total"
			strPropName = "Load Percentage"
'			intValue = Int(totalValue/objectCount)
			intValue = Int(intAverageValue)
			f_Display()
			...
			if (Ucase(instance) = "_TOTAL") then
				...
				Next
				strResultTemp1 = "Unknown - " & instance &" not exists."
...
'-------------------------------------------------------------------------------------------------
	Function f_GetPrefix(intValue)
		...
		If (Not(IsNumeric(intValue))) Then
			Wscript.Echo "Unknown - LoadPercentage cannot be determined"
			Wscript.Quit(intUnknown)
			Exit Function
		End If
...
'-------------------------------------------------------------------------------------------------
	Function f_ExecQuery()
		...
		For Each objWMI in colWMI
			intValueTemp = objWMI.Properties_("LoadPercentage")
			totalValue = totalValue + intValueTemp
			objectCount = objectCount + 1
		Next
		intAverageValue = totalValue/objectCount
...
'*************************************************************************************************
		...
		strCommandName="check_cpu_load_percentage.vbs"
		...
		strProp = "DeviceID, LoadPercentage"
	f_GetAllArg()
